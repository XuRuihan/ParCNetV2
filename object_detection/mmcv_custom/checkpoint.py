...
def load_checkpoint(model, filename, map_location="cpu", strict=False, logger=None):
    ...
    # resize complex weight
    gf_keys = [k for k in state_dict.keys() if "complex_weight" in k]
    for gf_key in gf_keys:
        gf_pretrained = state_dict[gf_key]
        gf_current = model.state_dict()[gf_key]
        if gf_pretrained.shape != gf_current.shape:
            logger.warning(
                f"Resize weight from {gf_pretrained.shape} to {gf_current.shape}"
            )
            w = F.interpolate(
                gf_pretrained.permute(3, 0, 1, 2),
                size=gf_current.shape[1:3],
                mode="bilinear",
                align_corners=True,
            ).permute(1, 2, 3, 0)
            state_dict[gf_key] = w
